...
pmap(
  ...
  bind_cols() %>% 
  bind_cols(Date = Z$Date, .) #%>% select(- tidyselect::all_of(paste0("X_", remove_cols)))
...
filtered_rows <- 
pmap_dbl(.l = list(condition, 1:nrow(X), nrow(X):1),
         .f = ~ (..1 == ..2 | ..1 == ..3)
)  
...
v <- 
which(!is.na(X_f$X_PAYEMS))

v[1]
v[length(v)]

spline(v, X_f$X_PAYEMS[v])
```
